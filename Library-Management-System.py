...
def new_user():                             #new user check and loading data
    while True:
        try: 
            checking = input('are you a new user : ')
            print("note: if you are not a new user then don't add y else all the files will be reset.")
            if checking == 'y' or checking == 'n' or checking == 'Y' or checking == 'n':
                break
            else:
    ...
    if checking == 'y' or checking == 'Y':
        ...
        f40.close()
        time.sleep(1)
        print('loading done !, You are ready to go! ')
        time.sleep(1)
        screens.cls()
        screens.start_screen()
    else:
        print('well, here you go experienced user, opening screen.')
        time.sleep(1)
        screens.cls()
        screens.start_screen()
        
class screens():                             #different screens
...
class admin():                               #admin commands
    def admin_menu():                        #admin options
        ...
        while True:
            try:
                ...
                if admin_choice > 8 or admin_choice == 0:
                    print("only  from 1 - 8 choices !!!!")
                else:
                    break
            except:
    ...
    def login():                             #admin login
        ...
        if aname == 'admin' and pname == 'BBPSPP':
            admin.admin_menu()
        else:
...
class student():                              #student management
...
class library():                              #issue and deposit
